...
function _shareBetsWA() {
  const user    = auth.currentUser;
  const name    = user?.displayName || 'Meu';
  const appUrl  = 'https://bolao2026-a76c7.web.app';
  const text    = `🏆 ${name} apostas no Bolão Copa 2026!\nVeja meus palpites e entre no bolão:\n${appUrl}`;
  window.open(`https://wa.me/?text=${encodeURIComponent(text)}`, '_blank', 'noopener');
}
...
function _renderMyBetsPage(groupBets, knockoutBets, results, uid) {
  ...
  let html = `<div class="mb-topbar">
    ...
    <div class="mb-topbar-actions">
      <button class="btn btn-secondary btn-sm" onclick="_exportMyBetsPDF('${uid}')">📥 PDF</button>
      <button class="btn btn-wa btn-sm" onclick="_shareBetsWA()">
